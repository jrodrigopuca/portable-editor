...
11. Todo flujo que cambia de documento o le pregunta algo al usuario sobre él (New, Open en todas sus formas, Save, Save As, `restoreSession`, el reload del poll) corre por la cola serial `exclusive()` de `main.ts` — uno a la vez, así el documento no puede cambiar bajo sus `await`. Las entradas públicas (`newFile`/`openFile`/`saveFile`/`saveFileAs`) encolan; adentro de un flujo encolado se llama a las `run*` (encolar desde adentro de la cola espera a sí mismo para siempre). `doc.gen`/`isStale()` quedan SOLO para los flujos de fondo que no pueden esperar turno: el stat del poll y el autosave. Cambiar de documento = `becomeDocument()` después del último `await` (trampas #37 y #56).
