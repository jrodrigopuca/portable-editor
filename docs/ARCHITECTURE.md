...
32. **`PendingFile` fusiona eventos `RunEvent::Opened` que colisionan antes del único poll de `startup_file()`, no los pisa.** Un segundo evento (dos "Open With" casi simultáneos en el arranque frío, antes de que el webview termine de cargar) encontraba el slot ya ocupado y lo sobreescribía en silencio — el primer archivo desaparecía sin dejar rastro. Ahora, si ya hay un `pending` cuando llega el segundo evento, se conserva el PRIMER target (mismo criterio que `.next()` dentro de un evento) y se suma `1 + extra_this_event` a su `extra_ignored` — no se emite `open-file` para el segundo evento porque en esa ventana nadie tiene el listener registrado todavía de cualquier forma. Esta colisión SOLO puede pasar antes de que `init()` llame `startup_file()` una vez (ese `.take()` deja `pending` en `None` para siempre); cualquier "Open With" con la app ya corriendo encuentra `pending` en `None` y sigue el camino normal (emite y pasa por `openFileQueue`, trampa #23) — esta trampa no le cambia nada a ese caso.
