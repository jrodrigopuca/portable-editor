...
4. ~~**Una recarga con contenido idéntico NO es no-op para el usuario**~~ ✅ 2026-08-25 — `reloadFromDisk()` llama `replaceText` solo si `file.contents !== editor.getText()`; los campos de disco se actualizan igual. Trampa #44 corregida.
5. ~~**`onCloseRequested` es el último diálogo fuera de la cola**~~ ✅ 2026-08-25 — cuerpo de `onCloseRequested` dentro de `await exclusive(...)`. Trampa #23 actualizada; invariante #11 sin excepciones.
6. ~~**`write_atomic` hace stat DESPUÉS del `rename` (`fs_ops.rs`)**~~ ✅ 2026-08-25 — `write_atomic` devuelve `mtime_ms(&tmp)` leído antes del `rename`. Trampa #44.
7. ~~**Mod+O / Mod+Shift+S repetidos con un diálogo nativo arriba encolan N diálogos en fila**~~ ✅ 2026-08-25 — `withPicker(task)` + `pickerPending` en los wrappers de `openFile` (sin path) y `saveFileAs`. Trampa #59.
8. ~~**`exclusive()` es el invariante que sostiene todo y tiene cero tests**~~ ✅ 2026-08-25 — `src/queue.ts` (`createSerialQueue`) con 4 tests: orden, aislamiento de rechazos, valor de retorno y el deadlock desde adentro documentado como test. `main.ts` lo consume.
9. ~~**Gusto**~~ ✅ 2026-08-25 — `beginDocument()` inlineado en `becomeDocument()`; nota HMR en trampa #32 (ítem 1); `reloadTransaction()` extraída pura de `editor.ts` con 3 tests (`editor-reload.test.ts`: tag, clamp de cursor, undo/redo tras reload). Trampa #60. Vitest: 75.
...
~~**Pregunta abierta (decisión del autor, contradice §5.4):**~~ ✅ 2026-08-26 — **resuelta con la opción C** (tokens explícitos): `ThemeChrome { background, foreground, border, field, accent }` en cada `ThemeDefinition` (One Dark y Paper con sus valores de siempre; Nord y Solarized con los de sus paletas oficiales — el chrome de Nord es más claro que su editor, lo que descarta derivarlo automáticamente); `applyTheme` publica `--chrome-*`; `styles.css` reemplaza los seis bloques `body[data-dark=…]` de status bar / búsqueda / atajos por una regla cada uno. `data-dark` queda solo para el `●`. §5.4 no se contradice: su objeción era a cuatro variantes de CSS, y con tokens el CSS es uno. Trampa #65; paso nuevo en la guía de temas de ARCHITECTURE.md. Verificar en vivo los cuatro temas (SMOKE-TEST).
